...
class PythonASTDocumentLoader(BaseLoader):
    ...
    def _extract_block_offsets(self, source_code) -> List[Tuple[int, int]]:
        """
        Returns as a sorted list of tuples containing the start and end offset of the block. 
        """
        tree = self.parser.parse(source_code, encoding="utf8")
    ...
    def _simplify_code(self, block_offsets, text_splitter: Optional[RecursiveCharacterTextSplitter] = None):
        """
        ...
        if text_splitter is not None:
            if isinstance(text_splitter, type):  # if its a class, instantiate
                splitter = text_splitter(chunk_size=256,
                                         chunk_overlap=16)
            else:
                splitter = text_splitter  # if its an instance, use it as is
