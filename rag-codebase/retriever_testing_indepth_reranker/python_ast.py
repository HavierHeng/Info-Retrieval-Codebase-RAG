...
class PythonASTDocumentLoader(BaseLoader):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        with open(self.file_path, "rb") as f:
            ...
            all_nodes_metadata = self.__extract_nodes(
                tree.root_node, code_file_bytes, "root", "")
            all_nodes_text, all_nodes_metadata = self.__simplify_metadata(
                all_nodes_metadata, code_file_bytes)
    ...
    def __extract_nodes(self, node: Node, source_code: bytes, parent_type: str = "root", parent_name: str = "") -> List[Dict]:
        ...
        def process_node(node):
            ...
            if node.type == "function_definition":
                node_metadata = self.__extract_function_details(
                    node, parent_name, parent_type)
            elif node.type == "class_definition":
                node_metadata = self.__extract_class_details(
                    node, parent_name, parent_type)
            else:
                node_metadata = self.__extract_other_details(
                    node, parent_name, parent_type)
        ...
        def process_child_nodes(node, curr_name):
            ...
            for child in node.children:
                if child.type in ["function_definition", "class_definition"]:
                    ...
                    result.extend(self.__extract_nodes(
                        child, source_code, parent_type=PY_MAPPING[node.type], parent_name=curr_name))
                else:
                    # Non functions and classes only get metadata extracted
                    if node.type not in ["function_definition", "class_definition"]:
                        # Safety check if current node is not a function or class
                        result.append(self.__extract_other_details(
                            child, parent_type=PY_MAPPING[node.type], parent_name=curr_name))
            return result
    ...
    def __extract_comments_docstrings(self, node: Node) -> Tuple[List[str], List[str]]:
        ...
        comments = [strip_single_line_comments(
            self.__get_node_text(c)) for c in captures.get("comment", [])]
        docstrings = [strip_docstring(self.__get_node_text(c))
                      for c in captures.get("docstring", [])]
        ...
        if node_body:
            for child in node_body.children:
                if child.type == "function_definition":
                    methods.append(self.__extract_function_details(
                        child, parent_class_name, PY_MAPPING[node.type]))
        return methods
    ...
    def __simplify_metadata(
        ...
        for node_data in nodes_metadata:
            match node_data["block_type"]:
                case "others":  # Merge code
                    ...
                    others.append(
                        source_code[node_data["start_offset"]: node_data["end_offset"]] + b'\n')

                case "function" | "class":
                    code_for_str = self.__generate_code_for_block(
                        node_data).encode()
                    ...
                    if node_data["block_type"] == "class":
                        ...
                        node_text = [
                            source_code[node_data["start_offset"]: node_data["end_offset"]]]

                        for node_method in node_methods:
                            # Add methods as their own standalone entries
                            all_nodes.append(node_method)
                            all_nodes_text.append(
                                source_code[node_method["start_offset"]: node_method["end_offset"]])

                            method_code_for_str = self.__generate_code_for_block(
                                node_method).encode()
        ...
        others_combined_text, others_metadata = self.__process_global_scope(
            others_combined_text, others_metadata, text_splitter)

        all_nodes.extend(others_metadata)
        all_nodes_text.extend(others_combined_text)

        # Sort nodes by 'start_offset'
        sorted_nodes = sorted(zip(all_nodes_text, all_nodes),
    ...
    def __process_global_scope(
        ...
        if text_splitter:
            ...
            if isinstance(text_splitter, type):
                splitter = text_splitter.from_language(
                    language=langchain_text_splitters.Language.JS)
            else:
